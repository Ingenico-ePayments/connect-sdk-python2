...
class PaymentsClientTest(unittest.TestCase):
    ...
    def test_create_rejected(self):
        ...
        with self.assertRaises(DeclinedPaymentException) as exception:
            client.merchant("merchantId").payments().create(request_body)
        self.assertIn("payment '000002000020142544360000100001'", str(exception.exception.message))
        self.assertIn("status 'REJECTED'", str(exception.exception))
        self.assertIn(response_body, str(exception.exception))
        self.assertIsNotNone(exception.exception.create_payment_result)
        self.assertEqual("000002000020142544360000100001", exception.exception.create_payment_result.payment.id)
        self.assertEqual("REJECTED", exception.exception.create_payment_result.payment.status)
